...
{
    public partial class branchesUC : UserControl
    {
        public branchesUC()
        {
            InitializeComponent();
        ...
        private void branchesUC_Load(object sender, EventArgs e)
        {
            retrieveData(1);
        }


        private void branchCheckBox_CheckStateChanged(object sender, EventArgs e)
        {
            if (branchCheckBox.CheckState == CheckState.Checked)
            {
                retrieveData(0);
            }
            else if (branchCheckBox.CheckState == CheckState.Unchecked || branchCheckBox.CheckState == CheckState.Indeterminate)
            {
                retrieveData(1);
            }
        }
        ...
        public void retrieveData(int isActive)
        {
            ...
            try
            {
                ...
                string query = "SELECT * FROM branches WHERE branches.active = " + isActive + " ORDER BY updated_at DESC";
                ...
                foreach (DataRow row in dataTable.Rows)
                {
                    ...
                    button.MouseClick += (sender3, e3) =>
                    {
                        ...
                        updateBranch form = new updateBranch();
                        form.ShowDialog();
                    };
